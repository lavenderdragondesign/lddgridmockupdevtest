...
import { UIWorkerMessageData, WorkerStatePayload, WorkerBitmapsPayload } from './types';

let ctx: OffscreenCanvasRenderingContext2D | null = null;
let canvas: OffscreenCanvas | null = null;
let cachedBitmaps: any = null;


self.onmessage = async (event: MessageEvent<UIWorkerMessageData>) => {
    ...
    if (type === 'bitmaps' && bitmaps) {
        cachedBitmaps = bitmaps;
        return;
    }

    if (type === 'draw' && ctx && canvas && state && cachedBitmaps && viewport) {
        if (canvas.width !== viewport.width || canvas.height !== viewport.height) {
            canvas.width = viewport.width;
            canvas.height = viewport.height;
        }
        try {
            await drawCanvas(ctx, canvas.width, canvas.height, state, cachedBitmaps);
        } catch (error) {
